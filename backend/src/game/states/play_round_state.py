...
class PlayRoundWithoutThrowState(GameState):
    ...
    def handle_input(self, player_input: PlayerInput) -> StateResponse:
        ...
        if is_defender_collecting and player_input.player_id == defender_id:
            return StateResponse(
                ActionResult.INVALID_ACTION,
                "Вы уже решили взять карты, ожидайте паса от атакующего.",
            )
        ...
        if player_input.player_id == defender_id:
            ...
            if player_input.action == PlayerAction.PASS:
                ...
                if not self.game.game_table.table_cards:
                    return StateResponse(
                        ActionResult.INVALID_ACTION,
                        "Нельзя пасовать, если на столе нет карт.",
                    )
        ...
        return StateResponse(
            ActionResult.INVALID_ACTION, "Нераспознанное действие или неверный ход."
        )
    ...
    def _check_attack_rules(self, player_input: PlayerInput) -> StateResponse:
        ...
        if not attacker or player_input.attack_card not in attacker.get_cards():
            ...
            logger.info(
                f"Card to play: {player_input.attack_card} (rank={player_input.attack_card.rank} id={id(player_input.attack_card.rank)} type={type(player_input.attack_card.rank)}), (suit={player_input.attack_card.suit} id={id(player_input.attack_card.suit)} type={type(player_input.attack_card.suit)})"
            )
            for c in attacker.get_cards():
                logger.info(
                    f"  Card in hand: {c} (rank={c.rank} id={id(c.rank)} type={type(c.rank)}), (suit={c.suit} id={id(c.suit)} type={type(c.suit)})"
                )
                logger.info(
                    f"    card == c: {player_input.attack_card == c}, card is c: {player_input.attack_card is c}, hash(card): {hash(player_input.attack_card)}, hash(c): {hash(c)}"
                )
                logger.info(
                    f"    card.rank module: {player_input.attack_card.rank.__module__}, c.rank module: {c.rank.__module__}"
                )
                logger.info(
                    f"    card.suit module: {player_input.attack_card.suit.__module__}, c.suit module: {c.suit.__module__}"
                )
            ...
            if self.game.round_defender_status == PlayerAction.COLLECT:
                ...
                if len(attack_cards_on_table) >= len(defender.get_cards()):
                    return StateResponse(
                        ...
                        "Нельзя подкинуть больше карт, чем есть у защищающегося.",
                    )
            # If the defender is still playing, they must have enough cards to beat the new attack.
            else:
                if not self._is_defender_able_to_beat_more(
                    defender_cards=defender.get_cards()
                ):
                    return StateResponse(
                        ActionResult.TABLE_FULL,
    ...
    def _are_all_cards_on_table_defended(self) -> bool:
        ...
        if not self.game.game_table.table_cards:
            return True
        return all(card.get("defend_card") for card in self.game.game_table.table_cards)
    ...
    def get_allowed_actions(self) -> Dict[str, List[str]]:
        ...
        if attacker_id in allowed_actions:
            ...
            if is_defender_collecting or (
                self.game.game_table.table_cards and all_cards_beaten
            ):
                allowed_actions[attacker_id].append(PlayerAction.PASS.name)

        # Determine Defender's actions
        if (
            defender_id in allowed_actions
            and not is_defender_collecting
            and not all_cards_beaten
        ):
            ...
            allowed_actions[defender_id].extend(
                [PlayerAction.DEFEND.name, PlayerAction.PASS.name]
            )
