...
def test_game_flow_until_card_draw(initialized_game: FoolGame):
    ...
    if synthesized_defend_card not in game.players[game.current_defender_idx].get_cards():
...
def test_game_flow_until_end_cond_demo(initialized_game: FoolGame):
    ...
    if game.deck.trump_suit == Suit.HEARTS:
        game.game_table.table_cards = [{"attack_card":TrumpCard(Rank.SEVEN, Suit.HEARTS)}]
    else:
        game.game_table.table_cards = [{"attack_card":Card(Rank.SEVEN, Suit.HEARTS)}]
    ...
    assert game.players[0].status == PlayerStatus.NOT_READY
    assert game.players[1].status == PlayerStatus.NOT_READY
