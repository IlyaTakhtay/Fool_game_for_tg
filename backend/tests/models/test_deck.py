...
from backend.src.game.models.card import Card, TrumpCard, Suit, Rank
...
def test_shuffle(deck):
    ...
    assert {(card.rank, card.suit) for card in deck._cards} == {
        (card.rank, card.suit) for card in cards_before
    }
...
def test_cards_uniqueness_and_completeness(deck):
    ...
    trump_suit = next(
        (card.suit for card in cards if isinstance(card, TrumpCard)), None
    )
    ...
    for rank in Rank:
        for suit in Suit:
            ...
            matching_cards = [
                card for card in cards if card.rank == rank and card.suit == suit
            ]
            # Проверяем, что такая карта ровно одна
            assert (
                len(matching_cards) == 1
            ), f"Ожидалась ровно одна карта для {rank} {suit}"

            # Проверяем тип карты в зависимости от масти
            if suit == trump_suit:
                assert isinstance(
                    matching_cards[0], TrumpCard
                ), f"Карта {matching_cards[0]} должна быть TrumpCard"
            else:
                assert not isinstance(
                    matching_cards[0], TrumpCard
                ), f"Карта {matching_cards[0]} не должна быть TrumpCard"
